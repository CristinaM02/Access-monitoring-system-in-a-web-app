            ...
            if ($tokenCheck === false) {
                ...
                if (!mysqli_stmt_prepare($stmt, $sql)){
                    ...
                    if (!$row = mysqli_fetch_assoc($results)) {
                        ...
                        $_SESSION['STATUS']['loginstatus'] = 'Invalid token, please use a new verification email!';
                        header("Location: ../");
                        exit();
                    }
                    else {

                        $sql = "UPDATE users SET VerificationDate=NOW(), is_verified='Yes' WHERE email=?;";
                        $stmt = mysqli_stmt_init($conn);
                        ...
                        if (!mysqli_stmt_prepare($stmt, $sql))
                        {
                            ...
                            if (!mysqli_stmt_prepare($stmt, $sql)){
                                ...
                                $_SESSION['STATUS']['loginstat'] = 'Account activated. Please log in!';
